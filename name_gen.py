...
class Culture_type(object):
    ...
    def __init__(self, name_parts=None):
        self.name_stems = []
        if name_parts == None:
            name_array = type(self).names
        else:
            name_array = name_parts
        if __debug__ == True: print('initialising name_gen')

        for entry in name_array:
            if __debug__ == True: print(entry)
            self.name_stems.append(entry)
            setattr(self, entry, name_array[entry])
        if __debug__ == True: print(self, "online.")
...
def person_name_gen(culture=Culture_type(), existing_names=[]):
    ...
    is_name = True
    while is_name == True:
        if __debug__ == True: print ("failed attempts: {}".format(attempt))
        ...
        if name not in existing_names:
            ...
            is_name = True
            attempt += 1
    if __debug__ == True: print (len(existing_names))
    return name
